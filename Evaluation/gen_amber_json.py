...
def main():
    ...
    ap.add_argument("--questions_json", required=True,
                    help="AMBER question file, JSON list")
    ap.add_argument("--image_root", required=True,
                    help="folder containing AMBER images")
    ...
    rows = json.load(open(args.questions_json, "r", encoding="utf-8"))
    ...
    for i, row in enumerate(rows):
        ...
        task = row.get("task", "disc")   # disc or gen
        ...
        out = {
            ...
            "task": task,
            "raw_output": pred,
            "answer": parse_yes_no(pred) if task == "disc" else pred,
            "label": row.get("label"),
            "objects_present": row.get("objects_present", []),
        }
        outputs.append(out)

        if (i + 1) % 100 == 0:
            print(f"[AMBER gen] {i+1}/{len(rows)}")
